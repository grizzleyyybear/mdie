...
class BaselineModel(nn.Module):
    """Backbone plus metric-learning/classification head for a SOTA baseline."""
...
def build_baseline(name: str, n_classes: int, embedding_dim: int = 512) -> BaselineModel:
    """Build one of the four baseline face-recognition models."""
    name = name.lower()
    if name == "facenet":
        return BaselineModel(
            build_backbone("facenet", embedding_dim=embedding_dim),
            TripletHead(margin=0.30),
            "triplet",
        )
    if name == "arcface":
        return BaselineModel(
            build_backbone("ir50", embedding_dim=embedding_dim),
            ArcFaceHead(embedding_dim, n_classes),
            "arcface",
        )
    if name == "cosface":
        return BaselineModel(
            build_backbone("ir50", embedding_dim=embedding_dim),
            CosFaceHead(embedding_dim, n_classes),
            "cosface",
        )
    if name == "mobilefacenet":
        return BaselineModel(
            build_backbone("mobilefacenet", embedding_dim=embedding_dim),
            ArcFaceHead(embedding_dim, n_classes),
            "arcface",
        )
