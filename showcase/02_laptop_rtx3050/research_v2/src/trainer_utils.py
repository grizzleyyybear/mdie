...
def set_lr(optim: torch.optim.Optimizer, lr: float) -> None:
    """Set the LR on every param group, honouring an optional per-group
    ``lr_scale`` so discriminative fine-tuning (e.g. a small backbone LR)
    survives the warmup-cosine schedule. Groups without ``lr_scale`` use 1.0."""
