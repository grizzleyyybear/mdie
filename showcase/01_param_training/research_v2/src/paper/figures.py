...
def plot_attention_overlay(image: np.ndarray, attn: np.ndarray, out: Path,
    ...
    # Robust normalisation: clip at the 99th percentile so a single hottest cell
    # does not dominate, then gamma-lift (<1) so the distributed secondary bone
    # regions (cheekbones, jaw angles, chin, nasal bridge) stay visible instead
    # of being washed out by min-max stretching onto the brightest peak only.
    a = np.clip(a, 0.0, None)
    hi = np.percentile(a, 99.0)
    a = a / (hi + 1e-8)
    a = np.clip(a, 0.0, 1.0) ** 0.6
